#!/usr/bin/env node
/**
 * obsidian-folder-tag-sync docs serve — interactive menu for local docs
 * preview + sharing.
 *
 ...
 * and the local Tailscale share workflow for mobile preview.
 */
...
function run(cmd) {
  try {
    return execSync(cmd, { stdio: 'pipe', timeout: 30000, shell: true }).toString().trim();
  } catch {
    return '';
  }
}
...
function hasCmd(name) {
  return isWindows ? !!run(`where ${name} 2>nul`) : !!run(`which ${name} 2>/dev/null`);
}
...
// "Cannot find module @rollup/rollup-<platform>". Detect and force reinstall.
...
async function prompt() {
  ...
  return new Promise((res) => {
    rl.question('  Choose [1-5]: ', (a) => {
      rl.close();
      const map = { 1: 'dev', 2: 'preview', 3: 'build', 4: 'tailscale', 5: 'cloudflare' };
      res(map[a.trim()] || 'dev');
    });
  });
}
...
function startDev() {
  ...
  const child = track(
    spawn('bun', ['x', 'astro', 'dev', '--host', '0.0.0.0', '--port', String(DOCS_PORT)], {
      cwd: docsDir,
      stdio: 'inherit',
      shell: isWindows,
    })
...
function startPreview() {
  ...
  return new Promise((res, rej) => {
    build.on('exit', (code) => {
      ...
      track(
        spawn('bun', ['x', 'astro', 'preview', '--host', '0.0.0.0', '--port', String(DOCS_PORT)], {
          cwd: docsDir,
          stdio: 'inherit',
          shell: isWindows,
        })
...
function startCloudflareTunnel(port) {
  ...
  return track(
    spawn('bun', ['x', 'cloudflared', 'tunnel', '--url', `http://localhost:${port}`], {
      stdio: 'inherit',
      shell: isWindows,
    })
  );
...
async function main() {
  ...
  switch (chosen) {
    ...
    default:
      ...
      console.error('Use one of: dev, preview, build, tailscale, cloudflare (or no arg for menu)');
      process.exit(1);
  }
}
