...
**Build status (current — v0.1.34):** the Scan & Snap drafting flow shipped first (0.1.29), then the **Taxonomy Workbench** direction took over (the term "Scan & snap" was renamed; the command is now "Taxonomy Workbench: draft rules from detected systems"). Shipped since:
...
**995 unit tests + E2E per surface (preview, composition, map) green; lint clean.** Also unrelated: GitHub issue #1 (duplicate frontmatter tags) was fixed + shipped (0.1.31) via the adversarial loop. Hot Reload is installed in the dev vault so builds drop straight in.

**Next slices** (all on existing seams): interaction on the Map (snap a system onto a branch → live coverage/conflict via `scanAndSnapPlan`); making composition INSTALLABLE (solve the deferred H1 precedence-persistence); then the wizard/WorkspaceConfig and export/establish. Full plan in the Taxonomy Workbench design entry.
