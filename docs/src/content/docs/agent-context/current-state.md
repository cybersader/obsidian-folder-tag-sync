...
**1008 unit tests + E2E per surface (preview, composition, map, sensing) green; lint clean.** Also unrelated: GitHub issue #1 (duplicate frontmatter tags) was fixed + shipped (0.1.31) via the adversarial loop. Hot Reload is installed in the dev vault so builds drop straight in.
