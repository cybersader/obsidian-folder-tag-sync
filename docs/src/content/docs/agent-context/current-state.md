...
**Build status:** **Phase 0 + Phase 1 shipped in v0.1.29** (BRAT) — see [Scan & Snap shipped](/obsidian-folder-tag-sync/agent-context/zz-log/2026-04-30-scan-and-snap-phase-1-shipped/). The `scan-and-snap-draft-rules` command ("Scan & snap: draft rules from my vault") scans the vault and drafts conflict-checked candidate rules from detected packs — the first time a user gets rules without writing regex. Phase 0 was pure engine refactors; Phase 1a the pure planner (`scanAndSnapPlan.ts`); Phase 1b the `ScanAndSnapModal` + command. 843 unit + 6 real-Obsidian E2E tests green; lint clean. **Phase 2 is next** — the SEACOW axis legend with open "snap" slots (activating the `compatibleWith`/`axes` pack metadata). Then Phase 3 (raw-structure synthesizer) and Phase 4 (establish + save-as-pack). All on existing seams (`detectPacks`, `scopeRules`, `previewRule`, `computeConflicts`, `deriveRule`, `extractInstances`).
